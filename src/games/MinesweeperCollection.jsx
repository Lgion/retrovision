...
const collections = {
  difficulty: [
    { id: 'facile', type: 'free', name: 'Facile (9x9, 10 Mines)' },
    { id: 'moyen', type: 'free', name: 'Moyen (12x12, 25 Mines)' },
    { id: 'difficile', type: 'free', name: 'Difficile (16x16, 40 Mines)' }
  ],
  theme: [
    { id: 'classic', type: 'free', name: 'Classique Windows' },
    { id: 'dark', type: 'free', name: 'Opération Nocturne' },
    { id: 'neon', type: 'free', name: 'Cyberspace' }
  ]
};

const MinesweeperCollection = ({ onClose, currentSelections, onSelect }) => {
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: '#1A1C29', zIndex: 1000, display: 'flex', flexDirection: 'column', color: 'white', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ padding: '20px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <h2 style={{ margin: 0, color: '#ef4444' }}>BOUTIQUE DÉMINEUR</h2>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '15px' }}>
          {collections[activeTab].map(item => {
            ...
            return (
              <div key={item.id} onClick={() => onSelect(activeTab, item.id)} style={{ background: '#2A2C39', padding: '15px', borderRadius: '12px', border: `2px solid ${isSelected ? '#ef4444' : 'transparent'}`, display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer', transition: 'all 0.2s', textAlign: 'center' }}>
                ...
                {isSelected && <div style={{ color: '#ef4444', marginTop: '5px', fontSize: '0.9rem' }}>✅ Actif</div>}
              </div>
            );
          })}
        </div>
...
export default MinesweeperCollection;
